...
The {\em MetaModelReader} parses the XML meta-model file and
...
First the components are started using {\tt
...
After external model start-up, the TLMManager waits until all external
models have registered themselves. This is performed in {\em
  ManagerCommHandler::RunStartupProtocol()}. Here an outline of the code:
{\scriptsize
\begin{verbatim}
...
The above method has two main tasks, first to check for new
