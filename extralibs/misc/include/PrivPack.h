...
// This define will also fix hetrogen in mpp_comm.h
#define HETROGEN
...
/*
   ...
   within the object.
   It can pack integers, floats, doubles, double3, double33, and 
   Bstring. Also packing and unpacking of arrays of integers, floats and doubles
...
struct packbuf_size_s {    
    ...
    packbuf_size_s(){ints=0; doubles=0; chars=0; floats=0; } ;
    packbuf_size_s(const int i, const int d, const int c, const int f){
        ints=i; doubles=d; chars=c; floats=f;
    } ;
#if 0    
    packbuf_size_s operator-(const packbuf_size_s &v){
        return packbuf_size_s(-v.doubles, -v.doubles, -v.chars, -v.floats) ;
    }
#endif
    int bytes(){
	return(ints*sizeof(int) + doubles*sizeof(double) + chars*sizeof(char) + floats*sizeof(float)) ;
    }
...
struct buftyp_s {
    ...
    char   *cptr ;    
    float  *fptr ;
    buftyp_s(){iptr=0; dptr=0; cptr=0; fptr=0; } ;
} ;
...
class aPackBuf {
    friend ostream& operator<<(ostream& o, const aPackBuf& v) ;
    friend ostream& operator<<(ostream& o, const aPackBuf* v) ;
...
public:    
    ...
    aPackBuf(const packbuf_size_t size) ;
    ...
    int UnPackedAllData(){
      ...
      if(a.doubles != received.doubles) return 0 ;
      if(a.ints    != received.ints   ) return 0 ;
      if(a.chars   != received.chars  ) return 0 ;
      if(a.floats != received.floats) return 0 ;
      return 1 ;
    }
    ...
    /* <msd type=public> 
     ...
     int, float, double, double3, double33, and Bstring.
    */
...
//! Packs three float.  //BZ536F
  ...
  inline void direct_arr_pk(const float *x, const int len) {
    ...
    */
    ...
    void pk(const vector<double> &x);
    //! Unpacks a  vector<double>.  BZ1374
    void upk(vector<double> &x);
    //! Packs a  vector<double>.  BZ1374
    void pk(const vector<vector<double> > &x);
    //! Unpacks a  vector<double>.  BZ1374
    void upk(vector<vector<double> > &x);
  ...
  //! (Un)packs a float33s.
    ...
    inline void pack(GeneralMatrix& x) { pack(x.Store(), x.Storage());};

    //! Pack vector of doubles, together with its length
    void pack(vector< double >  & x){
        assert(way != NoWay) ;
        if(way==Down) pk(x) ;
        else upk(x) ;
    };
    ...
    void pack(vector< vector< double > > & x){
        assert(way != NoWay) ;
        if(way==Down) pk(x) ;
        else upk(x) ;
    };
...
inline ostream& operator<<(ostream& o, aPackBuf *v){
  switch(v->getPackDir()){
  case Up:
    o << " Up " ;
    break;
