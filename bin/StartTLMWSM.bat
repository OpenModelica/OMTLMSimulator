...
set WSMPath=c:\Progra~2\Wolfra~1\System~1.0

rem *********************SETUP SIMULATION*********************************************

set WSM_Cmd=%WSMPath%/bin/WSMKernel.exe
...
echo loadFile("c:/Program Files (x86)/Wolfram Research/SystemModeler 4.0/Libraries/Modelica 3.2.1/Modelica/package.mo"); >> %MOSFILE%
echo loadFile("%TLMPluginPath%\Modelica\TLM.mo"); >> %MOSFILE%
rem echo loadModel(Modelica); >> %MOSFILE%
echo loadFile("%6"); >> %MOSFILE%
echo getErrorString(); >> %MOSFILE%
echo checkModel(%MODELNAME%); >> %MOSFILE%
echo getErrorString(); >> %MOSFILE%
rem echo simulate(%MODELNAME%, startTime=%2, stopTime=%3, numberOfIntervals=%INTERVALS%, tolerance=0.000001, method="Explicit Euler", outputFormat="plt", noClean=true); >> %MOSFILE%
echo getErrorString(); >> %MOSFILE%


echo Starting Wolfram SystemModeler
echo %WSM_Cmd% %MOSFILE%
echo DOING THIS: %WSM_Cmd% %1.mos > %1.simlog
%WSM_Cmd% %1.mos >> %1.simlog
echo DONE  THIS: %WSM_Cmd% %1.mos >> %1.simlog
