...
Force and torque variables must be specified as input and output variables at the top level of the model, in order to get the correct number of equations and variables.
Hence, we need to add the following variables and equations:
...
\item \textbf{Locate the TLM variables and write the value reference in \texttt{fmi.config}.}\\
...
\begin{lstlisting}[language=xml,float,floatplacement=ht,label={lst:metamodel},caption=Blabla...]
