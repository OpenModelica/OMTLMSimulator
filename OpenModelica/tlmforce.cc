                    ...
                    double ang_speed[])
{
    ...
    if( MarkerIDmap.find(markerID) != MarkerIDmap.end() ){
        ...
        if( interfaceID >= 0 ){
            TLMPluginStructObj->Plugin->SetMotion(interfaceID,          // Send data to the Plugin
						  simTime,
						  position,
						  orientation,
						  speed,
                    ...
                    double torque[])  // Output 3-component torque
{
    ...
    if( RHSFinalFlag ){
      ...
      set_tlm_motion(TLMPluginStructObj, markerID, simTime, position, orientation, speed, ang_speed);
    }
	...
	TLMPluginStructObj->Plugin->GetForce(interfaceID,
					     simTime,
					     position,
					     orientation,
					     speed,
