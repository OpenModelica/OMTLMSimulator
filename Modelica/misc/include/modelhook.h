...
public:
    ...
    //  virtual void set_name (const ModelNodeAdress & /* addr */, 
    // const Bstring) {return;};

    //! Return the name.
    virtual Bstring get_name(const ModelNodeAdress & /* addr */) {return Bstring("Noname");}

    //! Hook function informing that a sequence of pack_from_application
    //! calles related to the specified slave will follow.
    virtual void prepare_pack_from_app(const ModelNodeAdress & /* addr */, const int /* toSlave */ ){}

    //! Hook function informing that a sequence of pack_to_application
    //! calles related to the specified slave will follow.
    virtual void prepare_pack_to_app(const ModelNodeAdress & /* addr */, const int /* toSlave */ ){}
  
    //! Hook function packing data from the solver that is used
    //! for the compute_before method.
    virtual int pack_from_application(const ModelNodeAdress & /* addr */) {return 0;}

    //! Hook function packing the result from the compute_before method.
    virtual int pack_downward_output(const ModelNodeAdress & /* parent */, const ModelNodeAdress & /* child */) {return 0;}
  
    //! Hook function packing the result for the compute_after method.
    virtual int pack_upward_output(const ModelNodeAdress & /* child */, const ModelNodeAdress & /* parent */) {return 0;}

    //! Hook function packing the result of the compute_after method.
    //! This is the data that should be sent directly to the master.
    virtual int pack_to_application(const ModelNodeAdress & /* addr */) {return 0;}


    //! Do the before computation.
    virtual int compute_downward(const ModelNodeAdress & /* addr */) {return 0;}

    //! Do the before computation.
    virtual int compute_downward_while(const ModelNodeAdress & /* addr */) {return 0;}

    //! Do the after computation.
    virtual int compute_upward(const ModelNodeAdress & /* addr */) {return 0;}

    //! 
    virtual int get_n_child(const ModelNodeAdress & /* addr */) {return -1;}

    //! 
    virtual int get_n_element(const ModelNodeAdress & /* addr */) {return 1;}

    //! Get the type of the child connection.
    virtual int get_child_connection_type(const ModelNodeAdress &, int /*i*/) { return DEFAULT; }

    //! 
    virtual ModelHook * get_child(const ModelNodeAdress & /* addr */, int /*i*/) {return 0;}

    //! 
    virtual ModelNodeAdress get_child_node_adress(const ModelNodeAdress & /* addr */, int /*i*/ ) {return ModelNodeAdress();}



    //! 
    virtual int get_n_parent(const ModelNodeAdress & /* addr */) {return -1;}

    //! probably not neccessary. Nordling want it for Linear algebra
    //! traversing.
    virtual ModelHook * get_parent(const ModelNodeAdress & /* addr */, int /*i*/) {return 0;}
  
//
//  virtual ModelNodeAdress get_parent_node_adress (const ModelNodeAdress & 
//						    /* addr */, int i) 
//				   {return ModelNodeAdress();};
//  // probably not neccessary

    //! Set the first model node adress.
    void set_mna(const ModelNodeAdress & mna) { first_mna = mna; }

    //! Get the first model node adress.
    ModelNodeAdress get_mna() { return first_mna; }
  
    virtual ModelNodeAdress get_appl_mna() { return first_mna; }
